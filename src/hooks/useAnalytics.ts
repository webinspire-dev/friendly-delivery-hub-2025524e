...
export const trackEvent = async (courierId: string, eventType: EventType) => { try { await supabase.from('courier_analytics').insert({ courier_id: courierId, event_type: eventType, user_agent: navigator.userAgent }); } catch (e) { console.error('Error tracking:', e); } };
