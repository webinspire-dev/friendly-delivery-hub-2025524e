...
const HeroSection = () => {
  ...
  const { activeCities, isLoading: isLoadingCities, getCityDisplayName } = useCities();
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className={`space-y-8 ${direction === 'rtl' ? 'lg:order-2' : ''}`}>
            ...
            <div className="space-y-4 max-w-lg">
              ...
              >
                {isLocating ? (
                  <><Loader2 className="w-5 h-5 animate-spin" /><span>{direction === 'rtl' ? 'جاري تحديد موقعك...' : 'Localisation en cours...'}</span></>
                ) : (
                  <><Navigation className="w-5 h-5" /><span>{direction === 'rtl' ? 'استخدم موقعي الحالي' : 'Utiliser ma position actuelle'}</span></>
                )}
              </button>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className={selectedCity ? 'text-foreground' : 'text-muted-foreground'}>
                        {selectedCity ? getCityDisplayName(selectedCity) : t('hero.selectCity')}
                      </span>
                    </div>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-card border border-border rounded-xl shadow-lg overflow-hidden z-20 max-h-60 overflow-y-auto">
                      {isLoadingCities ? (
                        ...
                        activeCities.map((city) => (
                          <button
                            key={city.id}
                            onClick={() => { setSelectedCity(city.name); setDropdownOpen(false); }}
                            className="w-full px-5 py-3 text-left hover:bg-secondary transition-colors flex items-center gap-3"
                          >
                ...
                <Button size="lg" className="px-8 py-4 h-auto text-base font-semibold" onClick={handleSearch} disabled={!selectedCity}>
                  {t('hero.cta')}
                </Button>
              </div>
            </div>
